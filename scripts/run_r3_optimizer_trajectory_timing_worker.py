#!/usr/bin/env python3
"""Run one fresh R3-2B native or candidate wrapper timing worker."""
...
WORKER_SCHEMA = "boundflow.r3-2b-wrapper-timing-worker/v1"
WARMUP_COUNT = 3
SAMPLE_COUNT = 30
...
def _run(args: argparse.Namespace) -> dict[str, object]:
    if not torch.cuda.is_available():
        raise RuntimeError("R3-2B worker requires CUDA")
...
def main() -> None:
    ...
    if args.run_index < 0:
        raise ValueError("R3-2B worker run index differs")
    payload = _run(args)
    print(
        f"R3-2B run={payload['run_index']} mode={payload['mode']} "
