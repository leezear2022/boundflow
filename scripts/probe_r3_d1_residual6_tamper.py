#!/usr/bin/env python3
"""Probe fully re-signed D1-A residual6 artifact tampering."""
...
REPORT_SCHEMA = "boundflow.r3-d1a-residual6-tamper/v1"
...
def _json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise TypeError("R3-D1 residual6 tamper JSON differs")
    return value
...
def run(artifact: Path) -> dict[str, object]:
    cases: tuple[tuple[str, Callable[[Path], None]], ...] = (
        ...
        (
            ...
            lambda root: _mutate_summary(
                root, "d1b_schedule_qualification_open", False
            ),
        ),
        (
            "performance-claim",
    ...
    results = []
    with tempfile.TemporaryDirectory(prefix="boundflow-r3d1a-r6-tamper-") as temporary:
        for name, mutation in cases:
            ...
            try:
                ...
                error = str(caught)
            if not rejected:
                raise RuntimeError(f"R3-D1 residual6 tamper was accepted: {name}")
...
def main() -> None:
    ...
    replay(ARTIFACT)
    print(
        "R3-D1-A residual6 tamper PASS: "
        f"rejected={report['rejected_count']}/{report['case_count']}",
        flush=True,
    )
