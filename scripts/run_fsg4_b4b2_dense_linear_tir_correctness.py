"""Run the B4-B2 B2-1 five-fresh dense Linear correctness gate."""
...
from boundflow.runtime.fsg4_b4b2_dense_linear_tir import (
...
def run_gate() -> dict[str, object]:
    """Execute all five frozen S captures through one deterministic module cache."""

    if not torch.cuda.is_available():
        raise RuntimeError("B4-B2 dense Linear correctness requires CUDA")
    ...
    for run_ordinal in range(5):
        candidate = run_b4b2_dense_linear_tir_v1(
    ...
    return {
        "status": "validated-b2-1-dense-linear-correctness",
        "run_count": len(rows),
        "metric_count": metric_count,
        "element_count": 5 * (600 + 600 + 6 + 6144),
        ...
        "sparse_source_admitted": False,
    }


def main() -> None:
    """Print one canonical five-fresh correctness summary."""
