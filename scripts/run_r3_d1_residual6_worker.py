#!/usr/bin/env python3
"""Run one fresh D1-A residual6 staged correctness worker."""
...
WORKER_SCHEMA = "boundflow.r3-d1-residual6-worker/v1"
...
def _run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("R3-D1 residual6 worker requires CUDA")
    ...
    with torch.cuda.stream(stream):
        with tvm_ffi.use_torch_stream(torch.cuda.stream(stream)):
            ...
            input_payload = {
                "incoming": _cpu(s1),
                "weight4": _cpu(prepared._tensor("param/layer1.0.conv2.weight")),
                "lower19": _cpu(prepared._tensor("relu/19/lower").reshape(6, 1024)),
                "upper19": _cpu(prepared._tensor("relu/19/upper").reshape(6, 1024)),
                "alpha19": _cpu(prepared._tensor("relu/19/alpha")),
                "alpha_map19": _cpu(prepared.alpha_maps["19"]),
                "weight2": _cpu(prepared._tensor("param/layer1.0.conv1.weight")),
                "weight5": _cpu(prepared._tensor("param/layer1.0.shortcut.0.weight")),
                "bias4": _cpu(prepared._tensor("param/layer1.0.conv2.bias")),
                "bias2": _cpu(prepared._tensor("param/layer1.0.conv1.bias")),
                "bias5": _cpu(prepared._tensor("param/layer1.0.shortcut.0.bias")),
            ...
            reference_output = _cpu(s0[:12_288])
            reference_bias = _cpu(bias)
        scratch = torch.empty(6144, device="cuda")
        candidate_output = torch.empty(12_288, device="cuda")
    ...
    values = {
        ...
        "candidate_output": _cpu(candidate_output),
        "candidate_bias": _cpu(candidate_bias),
    }
...
def main() -> None:
    ...
    if args.run_index < 0:
        raise ValueError("R3-D1 residual6 worker run index differs")
    payload = _run(args)
    print(
        f"R3-D1-A residual6 run={payload['run_index']} timing_recorded=false "
        "performance_claimed=false",
        flush=True,
    )
