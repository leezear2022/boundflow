#!/usr/bin/env python3
"""Run or replay NRIR44 Phase-B projected-floor whole queries."""
...
FORMAL_SCHEMA_VERSION = "boundflow.root-projection-floor-global-formal/v1"
WORKER_SCHEMA_VERSION = "boundflow.root-projection-floor-global-worker/v1"
MANIFEST_SCHEMA_VERSION = "boundflow.root-projection-floor-global-manifest/v1"
ARTIFACT_DIR = Path(
...
MAX_COOPERATIVE_ELAPSED_NS = 48_000_000_000
MAX_FLOOR_ELAPSED_NS = 11_000_000_000
MAX_MEDIAN_RATIO = 0.82
...
def _code_revision() -> str:
    ...
    paths = (
        ...
        "boundflow/runtime/native_root_projection_multi_clause_anytime.py",
        ...
        "boundflow/runtime/native_prevalidated_objective_branch_shared_multi_clause_anytime.py",
...
def _source_phase_a() -> dict[str, Any]:
    ...
    if value.get("decision", {}).get("phase_a_go") is not True:
        raise ValueError("NRIR44B Phase-A admission differs")
    return value


def _source_nrir42() -> dict[str, Any]:
    from scripts.run_objective_branch_scorer_ownership_global_formal import (
        validate_formal as validate_nrir42,
    )

    value = _load_json(_repo_root() / SOURCE_NRIR42)
    validate_nrir42(value)
    if value.get("formal_payload_hash") != EXPECTED_SOURCE_NRIR42_HASH:
        raise ValueError("NRIR44B frozen NRIR42 source differs")
    return value
...
def _validate_worker(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or protocol.get("maximum_cooperative_elapsed_seconds") != 48
        ...
        or protocol.get("source_nrir42_formal_hash")
        != source_nrir42["formal_payload_hash"]
        ...
        != "root_projection_floor_fixed_resnet_admission_only"
        ...
        raise ValueError("NRIR44B worker result differs")
    from scripts.run_root_projection_floor_formal import (
...
def _worker(args: argparse.Namespace) -> None:  # pylint: disable=too-many-statements
    ...
    from boundflow.runtime.native_root_projection_multi_clause_anytime import (
        execute_native_root_projection_multi_clause_anytime_program,
    )
    ...
    query_id = f"nrir44:cifar10_resnet:000:property0:repeat{args.repeat_index}"
    ...
    wrapped = execute_native_root_projection_multi_clause_anytime_program(
        program,
        module,
        input_spec,
        linear_spec_C=tensors.linear_spec_c,
    ...
    result: dict[str, Any] = {
        ...
        "protocol": {
            ...
            "maximum_cooperative_elapsed_seconds": 48,
            ...
            "source_nrir42_formal_hash": _source_nrir42()["formal_payload_hash"],
        },
        "program": {
        ...
        "claim_boundary": ("root_projection_floor_fixed_resnet_admission_only"),
...
def _formal_payload(
    ...
    source_nrir42 = _source_nrir42()["formal_payload"]
    control = [int(value) for value in source_nrir42["whole_elapsed_ns"]]
    candidate = [int(row["runtime_trace"]["elapsed_ns"]) for row in results]
    ...
    timing_gate_passed = bool(
        all(value <= MAX_COOPERATIVE_ELAPSED_NS for value in candidate)
        and all(
    ...
    return {
        ...
        "source_nrir42_whole_elapsed_ns": control,
        "source_nrir42_whole_median_ns": control_median,
        "projected_whole_median_ns": candidate_median,
        "source_nrir42_whole_mad_ns": control_mad,
        "projected_whole_mad_ns": candidate_mad,
...
def validate_formal(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or value.get("source", {}).get("source_nrir42_formal_hash")
        != source_nrir42["formal_payload_hash"]
        or value.get("performance_claimed") is not False
        or value.get("claim") != "root_projection_floor_fixed_resnet_admission"
        or not isinstance(payload, dict)
    ):
        raise ValueError("NRIR44B formal envelope differs")
    results = payload.get("repeat_results")
    if not isinstance(results, list) or len(results) != REPEAT_COUNT:
        raise ValueError("NRIR44B formal repeat coverage differs")
    ...
    if (
        ...
        raise ValueError("NRIR44B formal gate differs")
...
def _generate(args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="boundflow-nrir44b-formal-") as temporary:
        ...
        for repeat in range(REPEAT_COUNT):
            ...
            if completed.returncode != 0 or not result_path.is_file():
                raise RuntimeError(
                    f"NRIR44B repeat {repeat} failed with {completed.returncode}: "
    ...
    formal = {
        ...
        "claim": "root_projection_floor_fixed_resnet_admission",
    ...
    if not isinstance(whole_elapsed_ns, list) or not all(
        ...
        raise TypeError("NRIR44B whole elapsed vector differs")
    print(
        _canonical_json(
            {
                ...
                "median_ratio": payload["projected_to_nrir42_median_ratio"],
...
def _replay(args: argparse.Namespace) -> None:
    ...
    if not isinstance(files, dict):
        raise TypeError("NRIR44B manifest files differ")
    ...
    if (
        ...
        raise ValueError("NRIR44B manifest/workload differs")
    print(
        _canonical_json(
            {
                "status": formal["status"],
