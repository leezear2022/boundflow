#!/usr/bin/env python3
"""Run or replay NRIR-42 Phase-B scorer-ownership whole queries."""
...
FORMAL_SCHEMA_VERSION = "boundflow.objective-branch-scorer-ownership-global-formal/v1"
WORKER_SCHEMA_VERSION = "boundflow.objective-branch-scorer-ownership-global-worker/v1"
MANIFEST_SCHEMA_VERSION = (
    "boundflow.objective-branch-scorer-ownership-global-manifest/v1"
)
ARTIFACT_DIR = Path(
...
def _code_revision() -> str:
    ...
    paths = (
        ...
        "scripts/run_objective_branch_scorer_ownership_global_formal.py",
    )
...
def _source_phase_a() -> dict[str, Any]:
    ...
    if value.get("decision", {}).get("phase_a_go") is not True:
        raise ValueError("NRIR-42B Phase-A admission differs")
    return value
...
def _validate_worker(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "objective_branch_scorer_ownership_global_admission_only"
        ...
        raise ValueError("NRIR-42B worker result differs")
...
def _worker(args: argparse.Namespace) -> None:  # pylint: disable=too-many-statements
    ...
    from boundflow.runtime.native_prevalidated_objective_branch_shared_multi_clause_anytime import (
        execute_native_prevalidated_objective_branch_shared_multi_clause_anytime_program,
    )
    ...
    query_id = f"nrir42:cifar10_resnet:000:property0:repeat{args.repeat_index}"
    ...
    execution = execute_native_prevalidated_objective_branch_shared_multi_clause_anytime_program(
        program,
        module,
        input_spec,
        linear_spec_C=tensors.linear_spec_c,
    ...
    result: dict[str, Any] = {
        ...
        "claim_boundary": ("objective_branch_scorer_ownership_global_admission_only"),
...
def validate_formal(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or value.get("performance_claimed") is not False
        or value.get("claim") != "objective_branch_scorer_ownership_global_admission"
        or not isinstance(payload, dict)
    ):
        raise ValueError("NRIR-42B formal envelope differs")
    results = payload.get("repeat_results")
    if not isinstance(results, list) or len(results) != REPEAT_COUNT:
        raise ValueError("NRIR-42B formal repeat coverage differs")
    ...
    if (
        ...
        raise ValueError("NRIR-42B formal gate differs")
...
def _generate(args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="boundflow-nrir42b-formal-") as temporary:
        ...
        for repeat in range(REPEAT_COUNT):
            ...
            if completed.returncode != 0 or not result_path.is_file():
                raise RuntimeError(
                    f"NRIR-42B repeat {repeat} failed with {completed.returncode}: "
    ...
    formal = {
        ...
        "claim": "objective_branch_scorer_ownership_global_admission",
    ...
    if not isinstance(whole_elapsed_ns, list) or not all(
        ...
        raise TypeError("NRIR-42B whole elapsed vector differs")
    print(
        _canonical_json(
            {
...
def _replay(args: argparse.Namespace) -> None:
    ...
    if not isinstance(files, dict):
        raise TypeError("NRIR-42B manifest files differ")
    ...
    if (
        ...
        raise ValueError("NRIR-42B manifest/workload differs")
    print(
        _canonical_json(
            {
                "status": formal["status"],
