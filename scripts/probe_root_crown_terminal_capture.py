#!/usr/bin/env python3
"""Capture one production root CROWN terminal Linear/ReLU transaction."""
...
from boundflow.runtime.root_crown_terminal_capture import (  # noqa: E402
...
def _worker(args: argparse.Namespace) -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("root CROWN terminal capture requires CUDA")
    capture = RootCrownTerminalCaptureV1()
    with tempfile.TemporaryDirectory(prefix="boundflow-root-terminal-") as raw:
    ...
    torch.save(
        {
            "schema_version": "boundflow.root-crown-terminal-tensors/v1",
...
def main() -> None:
    """Capture one production transaction and write tensors plus receipt."""
    _worker(_parse_args())
