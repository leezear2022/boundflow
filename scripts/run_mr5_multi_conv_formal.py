#!/usr/bin/env python3
"""Generate or replay the MR5 five-pair multi-Conv correctness artifact."""
...
from boundflow.runtime.mr5_multi_conv_formal import (  # noqa: E402
    EXPECTED_RUNS,
    FORMAL_SCHEMA,
    SOURCE_COMMIT,
    derive_summary,
...
CODE_PATHS = (
    "boundflow/backends/tvm/mr5_generalized_crown_conv.py",
    "boundflow/runtime/mr3_production_bridge_formal.py",
    "boundflow/runtime/mr5_generalized_crown_conv.py",
    "boundflow/runtime/mr5_multi_conv_production_bridge.py",
    "boundflow/runtime/mr5_multi_conv_formal.py",
    "scripts/run_mr5_multi_conv_production_bridge_worker.py",
    "scripts/run_mr5_multi_conv_formal.py",
    "scripts/probe_mr5_multi_conv_tamper.py",
    "tests/test_mr5_generalized_crown_conv.py",
    "tests/test_mr5_multi_conv_production_bridge.py",
    "tests/test_mr5_multi_conv_formal.py",
)
...
def _validate_code_revision(value: object) -> None:
    if value != _code_revision():
        raise ValueError("MR5 formal code revision differs")
...
def _worker_command(
    *,
    ...
    command = [
        str(python),
        str(ROOT / "scripts/run_mr5_multi_conv_production_bridge_worker.py"),
        "--benchmark-root",
        str(benchmark_root),
        "--abcrown-root",
        str(abcrown_root),
...
def _run_worker(command: list[str], *, environment: dict[str, str]) -> dict[str, Any]:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"MR5 formal worker failed with {result.returncode}:\n{result.stdout[-4000:]}"
        )
...
def replay_artifact(artifact: Path) -> dict[str, object]:
    ...
    if manifest.get("source_commit") != SOURCE_COMMIT:
        raise ValueError("MR5 formal manifest source differs")
    _validate_code_revision(manifest.get("code_revision"))
    files = manifest.get("files")
    if not isinstance(files, dict):
        raise TypeError("MR5 formal manifest files are absent")
...
def generate_artifact(args: argparse.Namespace) -> dict[str, object]:
    ...
    if _git("status", "--porcelain=v1", "--", *CODE_PATHS):
        raise ValueError("MR5 formal code paths are dirty")
    code_revision = _code_revision()
    environment = _worker_env()
    with tempfile.TemporaryDirectory(prefix="boundflow-mr5-formal-") as temporary:
        ...
        (staging / "README.md").write_text(
            ...
            "Five fresh provider/bridge pairs plus one C1 mutation-after-failure rollback probe.\n"
