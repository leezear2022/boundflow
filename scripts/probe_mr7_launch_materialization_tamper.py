#!/usr/bin/env python3
"""Run fully re-signed attacks against the MR7 attribution artifact."""
...
from scripts import run_mr7_launch_materialization_formal as formal  # noqa: E402

REPORT_SCHEMA = "boundflow.mr7-launch-materialization-tamper/v1"
...
def _host(raw: dict[str, Any]) -> None:
    receipt = _worker(raw, 0)["host_receipt"]
    receipt["category_ns"]["ffi_dlpack_stream"] += 1_000_000
    receipt["category_ns"]["optimizer_and_residual"] -= 1_000_000


def _device(raw: dict[str, Any]) -> None:
    _worker(raw, 1)["device_events"][0]["duration_ns"] += 1_000_000


def _marker(raw: dict[str, Any]) -> None:
    totals = _worker(raw, 1)["device_marker_totals"]
    first = sorted(totals)[0]
    totals[first] += 1_000_000
...
def _launch_count(raw: dict[str, Any]) -> None:
...
def _performance(raw: dict[str, Any]) -> None:
    _worker(raw, 0)["performance_claimed"] = True
...
ATTACKS: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
    ("host-category-full-resign", _host),
    ("device-duration-full-resign", _device),
    ("device-marker-full-resign", _marker),
    ("semantic-full-resign", _semantic),
    ("launch-count-full-resign", _launch_count),
...
def probe(artifact: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="boundflow-mr7-tamper-") as root:
        for ordinal, (name, attack) in enumerate(ATTACKS):
            ...
            if not rejected:
                raise ValueError(f"MR7 tamper accepted: {name}")
            results.append(
