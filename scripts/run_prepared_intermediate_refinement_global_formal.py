#!/usr/bin/env python3
"""Run or replay NRIR45 Phase-B prepared-refinement whole queries."""
...
FORMAL_SCHEMA_VERSION = "boundflow.prepared-refinement-global-formal/v1"
WORKER_SCHEMA_VERSION = "boundflow.prepared-refinement-global-worker/v1"
MANIFEST_SCHEMA_VERSION = "boundflow.prepared-refinement-global-manifest/v1"
ARTIFACT_DIR = Path(
    "artifacts/prepared-intermediate-refinement/"
    "vnncomp21-resnet2b-property0-three-repeat-cpu-phase-b-v1"
)
SOURCE_PHASE_A = Path(
    "artifacts/prepared-intermediate-refinement/"
...
MAX_COOPERATIVE_ELAPSED_NS = 40_000_000_000
MAX_MEASURED_ELAPSED_NS = 50_000_000_000
MAX_FLOOR_ELAPSED_NS = 11_000_000_000
MAX_TRACE_MEDIAN_RATIO = 0.90
MAX_MEASURED_MEDIAN_RATIO = 0.85
EXPECTED_SELECTED = [2, 3]
EXPECTED_WORST_ACTIVE_LOWERS = [-35.53092575073242, -30.258447647094727]
...
def _source_phase_a() -> dict[str, Any]:
    from scripts.run_prepared_intermediate_refinement_formal import (
        validate_formal as validate_phase_a,
    )

    value = _load_json(_repo_root() / SOURCE_PHASE_A)
    validate_phase_a(value)
    if value.get("decision", {}).get("phase_a_go") is not True:
        raise ValueError("NRIR45B Phase-A admission differs")
    return value


def _source_nrir44() -> dict[str, Any]:
    from scripts.run_root_projection_floor_global_formal import (
        validate_formal as validate_nrir44,
    )

    value = _load_json(_repo_root() / SOURCE_NRIR44)
    validate_nrir44(value)
    if value.get("formal_payload_hash") != EXPECTED_SOURCE_NRIR44_HASH:
        raise ValueError("NRIR45B frozen NRIR44 source differs")
    return value
...
def _correctness_gate(value: Mapping[str, Any]) -> bool:
    ...
    checks = value["checks"]
    prepared_refinement = value["prepared_refinement"]
    ...
    if (
        ...
        or prepared_refinement["capsule_count"] != 60
        or prepared_refinement["full_replay_count"] != 60
        or len(prepared_refinement["capsule_hashes"]) != 60
        or not all(_is_sha256(item) for item in prepared_refinement["capsule_hashes"])
    ...
    for position, item in enumerate(slices):
        ...
        if (
            ...
            or packed["capsule_count"] != len(bindings)
            or packed["compile_enumeration_count"] != len(bindings)
            or packed["execute_enumeration_count"] != 0
            or packed["prepared_refinement_capsule_count"] != 30
            or packed["candidate_source"] != "plan_owned_immutable"
...
def _production_gate(value: Mapping[str, Any]) -> bool:
    ...
    slices = value["slices"]
    return bool(
        value["floor"]["elapsed_ns"] <= MAX_FLOOR_ELAPSED_NS
        and value["runtime_trace"]["elapsed_ns"] <= MAX_COOPERATIVE_ELAPSED_NS
        and value["runtime_trace"]["measured_elapsed_ns"] <= MAX_MEASURED_ELAPSED_NS
        and all(
            item["packed"] is not None
            and item["slice"]["accepted_nodes"] == 31
            and item["slice"]["sibling_group_count"] == 15
            and item["packed"]["worst_active_lower"]
            == EXPECTED_WORST_ACTIVE_LOWERS[position]
...
def _validate_worker(value: Mapping[str, Any]) -> None:
    ...
    phase_a = _source_phase_a()
    source_nrir44 = _source_nrir44()
    if (
        ...
        or protocol.get("maximum_cooperative_elapsed_seconds") != 40
        or protocol.get("maximum_measured_elapsed_seconds") != 50
        or protocol.get("maximum_floor_elapsed_seconds") != 11
        ...
        or protocol.get("phase_a_formal_hash") != phase_a["formal_hash"]
        or protocol.get("source_nrir44_formal_hash")
        != source_nrir44["formal_payload_hash"]
        or not isinstance(value.get("floor_projection"), dict)
        or value.get("performance_claimed") is not False
        or value.get("claim_boundary")
        != "prepared_refinement_fixed_resnet_admission_only"
        ...
        raise ValueError("NRIR45B worker result differs")
    from scripts.run_root_projection_floor_formal import (
        _validate_projection_payload,
    )

    _validate_projection_payload(value["floor_projection"])
...
def _worker(args: argparse.Namespace) -> None:  # pylint: disable=too-many-statements
    ...
    from boundflow.runtime.native_prepared_root_projection_multi_clause_anytime import (
    ...
    from scripts.run_root_projection_floor_formal import _projection_payload
    ...
    query_id = f"nrir45:cifar10_resnet:000:property0:repeat{args.repeat_index}"
    ...
    wrapped = execute_native_prepared_root_projection_multi_clause_anytime_program(
        program,
        module,
        input_spec,
        linear_spec_C=tensors.linear_spec_c,
    ...
    execution = wrapped.source_execution
    prepared_executions = [
        node.execution
        for item in execution.packed_executions
        if item.packed is not None
        for node in item.packed.node_refinements
        if isinstance(node.execution, NativePreparedIntermediateRefinementExecution)
    ]
    single_input = _repeat_box_input_spec(input_spec, count=1)
    for prepared_execution in prepared_executions:
        validate_native_prepared_intermediate_refinement_full(
            prepared_execution, module, single_input
        )
    prepared_capsule_hashes = [
        item.program.capsule.stable_hash() for item in prepared_executions
    ]
    sources = []
    ...
    for item in execution.packed_executions:
        ...
        if packed is not None and item.verdict is not None:
            ...
            packed_summary = {
                ...
                "capsule_count": len(packed.queue.objective_branch_executions),
                "compile_enumeration_count": sum(
                    getattr(branch.program, "capsule").compile_enumeration_count
                    for _node_id, branch in packed.queue.objective_branch_executions
                ),
                "execute_enumeration_count": sum(
                    getattr(branch.program, "capsule").execute_enumeration_count
                    for _node_id, branch in packed.queue.objective_branch_executions
                ),
                "prepared_refinement_capsule_count": sum(
                    isinstance(
                        node.execution,
                        NativePreparedIntermediateRefinementExecution,
                    )
                    for node in packed.node_refinements
                ),
                "candidate_source": "plan_owned_immutable",
    ...
    result: dict[str, Any] = {
        ...
        "protocol": {
            ...
            "maximum_cooperative_elapsed_seconds": 40,
            "maximum_measured_elapsed_seconds": 50,
            "maximum_floor_elapsed_seconds": 11,
            ...
            "phase_a_formal_hash": phase_a["formal_hash"],
            "source_nrir44_formal_hash": _source_nrir44()["formal_payload_hash"],
        },
        "program": {
        ...
        "floor_projection": _projection_payload(wrapped.floor_projection),
        ...
        "prepared_refinement": {
            "capsule_count": len(prepared_executions),
            "full_replay_count": len(prepared_executions),
            "capsule_hashes": prepared_capsule_hashes,
        },
        "checks": {
            ...
            "scorer_ownership_not_erased": all(
                item.packed is None
                or all(
                    getattr(branch.program, "capsule").candidate_source
                    == "plan_owned_immutable"
                    and getattr(branch.program, "capsule").compile_enumeration_count
                    == 1
                    and getattr(branch.program, "capsule").execute_enumeration_count
                    == 0
                    for _node_id, branch in item.packed.queue.objective_branch_executions
                )
                for item in execution.packed_executions
            ),
            "prepared_refinement_not_erased": len(prepared_executions) == 60
            and all(
                item.program.capsule.full_validation_count == 1
                and item.program.prepared_schedule.runtime_target_selection_launches
                == 1
                for item in prepared_executions
            ),
        },
        "correctness_gate_passed": False,
        "production_gate_passed": False,
        "claim_boundary": ("prepared_refinement_fixed_resnet_admission_only"),
...
def _formal_payload(
    ...
    source_nrir44 = _source_nrir44()["formal_payload"]
    control_trace = [int(value) for value in source_nrir44["whole_elapsed_ns"]]
    control_measured = [
        int(row["runtime_trace"]["measured_elapsed_ns"])
        for row in source_nrir44["repeat_results"]
    ]
    candidate_trace = [int(row["runtime_trace"]["elapsed_ns"]) for row in results]
    candidate_measured = [
        int(row["runtime_trace"]["measured_elapsed_ns"]) for row in results
    ]

    def metric(control: list[int], candidate: list[int]) -> dict[str, Any]:
        control_median = int(statistics.median(control))
        candidate_median = int(statistics.median(candidate))
        control_mad = int(
            statistics.median(abs(value - control_median) for value in control)
        )
        candidate_mad = int(
            statistics.median(abs(value - candidate_median) for value in candidate)
        )
        improvement = control_median - candidate_median
        return {
            "source_nrir44_elapsed_ns": control,
            "prepared_elapsed_ns": candidate,
            "source_nrir44_median_ns": control_median,
            "prepared_median_ns": candidate_median,
            "source_nrir44_mad_ns": control_mad,
            "prepared_mad_ns": candidate_mad,
            "prepared_to_nrir44_median_ratio": candidate_median / control_median,
            "median_improvement_ns": improvement,
            "improvement_exceeds_pooled_mad": improvement
            > max(control_mad, candidate_mad),
        }

    trace_metric = metric(control_trace, candidate_trace)
    measured_metric = metric(control_measured, candidate_measured)
    timing_gate_passed = bool(
        all(value <= MAX_COOPERATIVE_ELAPSED_NS for value in candidate_trace)
        and all(value <= MAX_MEASURED_ELAPSED_NS for value in candidate_measured)
        and all(
            int(row["floor"]["elapsed_ns"]) <= MAX_FLOOR_ELAPSED_NS for row in results
        )
        and float(trace_metric["prepared_to_nrir44_median_ratio"])
        <= MAX_TRACE_MEDIAN_RATIO
        and float(measured_metric["prepared_to_nrir44_median_ratio"])
        <= MAX_MEASURED_MEDIAN_RATIO
        and bool(trace_metric["improvement_exceeds_pooled_mad"])
        and bool(measured_metric["improvement_exceeds_pooled_mad"])
    )
    worker_production_gates = all(
    ...
    return {
        ...
        "protocol": {
            ...
            "maximum_measured_elapsed_seconds": 50,
            "maximum_floor_elapsed_seconds": 11,
            "maximum_trace_median_ratio": MAX_TRACE_MEDIAN_RATIO,
            "maximum_measured_median_ratio": MAX_MEASURED_MEDIAN_RATIO,
        },
        ...
        "prepared_refinement_capsule_counts": [
            int(row["prepared_refinement"]["capsule_count"]) for row in results
        ],
        "prepared_refinement_full_replay_counts": [
            int(row["prepared_refinement"]["full_replay_count"]) for row in results
        ],
        "timing_gate_passed": timing_gate_passed,
        "all_worker_production_gates_passed": worker_production_gates,
        "all_correctness_gates_passed": all(
            bool(row["correctness_gate_passed"]) for row in results
        ),
        "all_production_gates_passed": worker_production_gates and timing_gate_passed,
    }
...
def validate_formal(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or value.get("status") not in {"validated-reduced", "validated-no-go"}
        or value.get("source", {}).get("native_code_revision") != _code_revision()
        or value.get("source", {}).get("phase_a_formal_hash") != phase_a["formal_hash"]
        or value.get("source", {}).get("source_nrir44_formal_hash")
        != source_nrir44["formal_payload_hash"]
        or value.get("performance_claimed") is not False
        or value.get("claim") != "prepared_refinement_fixed_resnet_admission"
        or not isinstance(payload, dict)
    ):
        raise ValueError("NRIR45B formal envelope differs")
    results = payload.get("repeat_results")
    if not isinstance(results, list) or len(results) != REPEAT_COUNT:
        raise ValueError("NRIR45B formal repeat coverage differs")
    ...
    if (
        ...
        or value.get("status")
        != ("validated-reduced" if production else "validated-no-go")
    ):
        raise ValueError("NRIR45B formal gate differs")
...
def _generate(args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="boundflow-nrir45b-formal-") as temporary:
        ...
        for repeat in range(REPEAT_COUNT):
            ...
            if completed.returncode != 0 or not result_path.is_file():
                raise RuntimeError(
                    f"NRIR45B repeat {repeat} failed with {completed.returncode}: "
    ...
    formal = {
        ...
        "status": "validated-reduced" if passed else "validated-no-go",
        "source": {
            "native_code_revision": _code_revision(),
            "phase_a_formal_hash": _source_phase_a()["formal_hash"],
            "source_nrir44_formal_hash": _source_nrir44()["formal_payload_hash"],
        },
        "claim": "prepared_refinement_fixed_resnet_admission",
    ...
    if not isinstance(whole_elapsed_ns, list) or not all(
        ...
        raise TypeError("NRIR45B whole elapsed vector differs")
    trace_metric = payload["trace_metric"]
    measured_metric = payload["measured_metric"]
    if not isinstance(trace_metric, dict) or not isinstance(measured_metric, dict):
        raise TypeError("NRIR45B metric payload differs")
    print(
        _canonical_json(
            {
                ...
                "trace_median_ratio": trace_metric["prepared_to_nrir44_median_ratio"],
                "measured_median_ratio": measured_metric[
                    "prepared_to_nrir44_median_ratio"
                ],
                "timing_gate_passed": payload["timing_gate_passed"],
            }
        )
    )
...
def _replay(args: argparse.Namespace) -> None:
    ...
    if not isinstance(files, dict):
        raise TypeError("NRIR45B manifest files differ")
    ...
    if (
        ...
        raise ValueError("NRIR45B manifest/workload differs")
    print(
        _canonical_json(
            {
                "status": formal["status"],
