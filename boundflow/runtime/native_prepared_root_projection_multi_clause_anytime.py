"""NRIR44 projected floor composed with NRIR45 prepared refinement."""
...
from .native_prepared_objective_branch_shared_production_queue import (
...
class NativePreparedRootProjectionMultiClauseExecution:
...
def _execute_native_prepared_root_projection_multi_clause_source(  # pylint: disable=too-many-statements
    ...
    for position in range(program.plan.allocation_policy.max_selected_clauses):
        ...
        if prior_unsafe:
            ...
            if compile_finished_ns >= cutoff_ns:
                ...
                try:
                    packed = execute_native_prepared_objective_branch_shared_production_queue(
                        composite_plan,
                        module,
                        input_spec,
                        linear_spec_C=objective,
...
def execute_native_prepared_root_projection_multi_clause_anytime_program(
    program: NativeMultiClauseAnytimeProgram,
    module: BFTaskModule,
    input_spec: InputSpec,
    *,
    ...
    branch_policy: NativeObjectiveBranchPolicy,
    clock_ns: ClockNs = time.monotonic_ns,
) -> NativePreparedRootProjectionMultiClauseExecution:
    """Execute NRIR42 with the ranking-only NRIR44 floor projection."""

    source, floor_projection = (
        _execute_native_prepared_root_projection_multi_clause_source(
            program,
            module,
            input_spec,
            linear_spec_C=linear_spec_C,
    ...
    execution = NativePreparedRootProjectionMultiClauseExecution(
        source, floor_projection
    )
    execution.validate_against(
        module,
