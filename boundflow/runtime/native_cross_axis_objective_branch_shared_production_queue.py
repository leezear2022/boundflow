"""Production queue using cross-axis batched objective-branch candidates."""
...
from .native_cross_axis_prevalidated_objective_branch_shared_evaluator import (
...
class NativeCrossAxisSharedParametricAncestralExecution:
    """Historical queue projection plus every physical scorer batch."""

    base: NativeSharedParametricAncestralExecution
    scorer_batches: Tuple[NativeCrossAxisPrevalidatedObjectiveBranchBatchExecution, ...]
    ...
    def validate_against(
        ...
        *,
        ...
        if not self.scorer_batches:
            raise ValueError("cross-axis production has no scorer batch")
        for scorer in self.scorer_batches:
            scorer.validate()
        committed_scorer_count = len(self.base.batch_commits)
        if len(self.scorer_batches) not in {
            committed_scorer_count,
            committed_scorer_count + 1,
        }:
            raise ValueError("cross-axis production scorer coverage differs")
        projected = tuple(
            execution.trace.stable_hash(program=execution.program)
            for scorer in self.scorer_batches[:committed_scorer_count]
            for execution in scorer.executions
        )
        queue_hashes = tuple(
            execution.trace.stable_hash(program=execution.program)
            for _node_id, execution in self.base.queue.objective_branch_executions
        )
        if projected != queue_hashes:
            raise ValueError("cross-axis production branch projection differs")
    ...
    @property
    def compiler_batches(self):
        return self.base.compiler_batches

    @property
    def batch_commits(self):
        return self.base.batch_commits

    @property
    def node_refinements(self):
        return self.base.node_refinements

    @property
    def discarded_compiler_batch(self):
        return self.base.discarded_compiler_batch
...
def execute_native_cross_axis_objective_branch_shared_production_queue(  # pylint: disable=too-many-statements
    ...
    *,
...
) -> NativeCrossAxisSharedParametricAncestralExecution:
    """Execute compile-owned objective branch scores in the real queue."""

    if not query_id or clause_ordinal < 0:
    ...
    root_values, root_branches, root_scorer_batch = (
        bind_cross_axis_prevalidated_objective_branches(
            tuple(
                NativeCrossAxisSharedObjectiveBranchInput(
                ...
                for position, item in enumerate(raw_root)
            ),
            module,
            input_spec,
            optimizer_policy=optimizer_policy,
    ...
    scorer_batches = [root_scorer_batch]
    ...
    while heap and not budget_exhausted and not deadline_exhausted:
        ...
        child_values, child_branches, child_scorer_batch = (
            bind_cross_axis_prevalidated_objective_branches(
                tuple(
                    NativeCrossAxisSharedObjectiveBranchInput(
                        clause_ordinal=clause_ordinal,
                    ...
                    for position, item in enumerate(raw_children)
                ),
                module,
                input_spec,
                optimizer_policy=optimizer_policy,
        ...
        scorer_batches.append(child_scorer_batch)
    ...
    execution = NativeCrossAxisSharedParametricAncestralExecution(
        base=base,
        scorer_batches=tuple(scorer_batches),
    )
