"""NRIR44 projected floor composed with the NRIR42 global runtime."""
...
from .native_prevalidated_objective_branch_shared_production_queue import (
...
from .native_root_projection_floor import (
...
class NativeRootProjectionMultiClauseExecution:
    """NRIR42 global execution with an explicitly owned NRIR44 floor."""

    source_execution: NativeSharedParametricMultiClauseExecution
    floor_projection: NativeRootProjectionFloorExecution
    ...
    def validate_against(
        ...
        *,
        ...
        self.floor_projection.validate_against(
            module,
            input_spec,
            linear_spec_C=linear_spec_C,
            thresholds=thresholds,
        ...
        if (
            self.source_execution.floor != self.floor_projection.source_execution
            or self.source_execution.program.floor_program
            != self.floor_projection.program.source_program
        ):
            raise ValueError("root-projection global execution/floor differs")


def _execute_native_root_projection_multi_clause_source(  # pylint: disable=too-many-statements
    ...
    *,
    ...
    NativeSharedParametricMultiClauseExecution, NativeRootProjectionFloorExecution
]:
    ...
    projection_program = lower_native_root_projection_floor_program(
        program.floor_program,
        linear_spec_C=objectives,
        thresholds=thresholds,
        plan_id=f"{program.plan.plan_id}:root-projection-nrir44",
    )
    started_ns = clock_ns()
    ...
    floor_projection = execute_native_root_projection_floor_program(
        projection_program,
        module,
        input_spec,
        linear_spec_C=objectives,
        thresholds=thresholds,
        query_id=f"{query_id}:floor-root-projection-nrir44",
        search_policy=search_policy,
        optimizer_policy=optimizer_policy,
        clock_ns=floor_clock_ns,
    )
    floor = floor_projection.source_execution
        ...
        if prior_unsafe:
            ...
            composite_plan = compile_native_objective_branch_shared_plan(
                ...
                plan_id=(
                    ...
                    "objective-branch-scorer-ownership-nrir42"
                ),
            )
            ...
            if compile_finished_ns >= cutoff_ns:
                ...
                try:
                    packed = execute_native_prevalidated_objective_branch_shared_production_queue(
                        composite_plan,
                        module,
                        input_spec,
                        linear_spec_C=objective,
    ...
    return execution, floor_projection


def execute_native_root_projection_multi_clause_anytime_program(
    program: NativeMultiClauseAnytimeProgram,
    module: BFTaskModule,
    input_spec: InputSpec,
    *,
    ...
    branch_policy: NativeObjectiveBranchPolicy,
    clock_ns: ClockNs = time.monotonic_ns,
) -> NativeRootProjectionMultiClauseExecution:
    """Execute NRIR42 with the ranking-only NRIR44 floor projection."""

    source, floor_projection = _execute_native_root_projection_multi_clause_source(
        program,
        module,
        input_spec,
        linear_spec_C=linear_spec_C,
    ...
    execution = NativeRootProjectionMultiClauseExecution(source, floor_projection)
    execution.validate_against(
        module,
        input_spec,
        linear_spec_C=linear_spec_C,
