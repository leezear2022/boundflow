...
2. 计时是 internal production admission，不是 competitor end-to-end；
3. prepare-once 的安全性依赖 exact owner、容器成员 identity 与 Tensor `_version`；请重点测试
   alias、替换和 mutation；
4. fast path 缓存 admitted hash，审计必须确认 full replay 没有错误复用 fast path；
5. final 仍 9/9 unknown；
6. 没有 fresh CUDA/GPU、multi-workload 或公平比较证据；
7. 当前约 31.3 秒 residual path 尚未完成新一轮 phase attribution，不能事后把任何下一步优化归因
   给 NRIR45。

## 7. 审计报告格式

请在 `.docops/exchange/nrir45-20260805/r001/` 返回：

1. 总体 verdict：`approve` 或 `changes_requested`；
2. AC1—AC6 逐项 verdict、独立命令与原始数字；
3. blocker / major / minor / info finding，含文件位置和可执行修复；
4. artifact tamper 探针的方法与结果；
