...
| C2：Method/Autograd/Memory-Aware Materialization Planner | IR-2 Plan IR + IR-3 Schedule reference closure validated-reduced；runtime/backend integration pending | typed PlanTemplate/PlanInstance、Task/Schedule lowering、跨 decision/verifier、state-validity、trace/replay | 21 个 Plan tests + 24 个 Task/Schedule/artifact tests；预算/OOM/event/query/state/replay | 历史 1,416 executions/23 feasible 保留；PR-11/12 raw decision records 当前为 0，不能声称逐记录迁移 |
...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4D typed plain-CROWN query/state path validated；legacy α/β closure pending | composite typed registry + backend-specific Task identity + real fused/unfused execution + semantic fallback + query→Plan/Task/Schedule + exact state task skip | MLP/CNN PyTorch/TVM、structured/chunked、cache/fallback、query order、state hit/stale、external No-Go | dispatch/cache v2 与 IR-4D query/state artifact 均可 fresh-process replay；PR-13 α/β compiler migration 不成立 |
