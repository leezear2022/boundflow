...
| BoundFlow Schedule IR | IR-3B control/executor/trace foundation validated；typed Task IR pending | typed ScheduleModule + batch/event/state/retry/replan + reference executor；旧 topo loop 仅作对照 | 10 个 lowering/control/trace/artifact tests；OOM bounded、stream happens-before、query accounting、Bound oracle 对齐 | deterministic dump/hash/trace/fresh-process replay 已有；逐 Task/backend E2E 尚缺 |
