...
| C2：Method/Autograd/Memory-Aware Materialization Planner | IR-4 closure + IR-5C2 typed MLP held-out PARTIAL；IR-5 未关闭 | typed PlanTemplate/PlanInstance、Task/Schedule lowering、adaptive context/evaluator、calibration-only CUDA runner | 8/8 Global feasible、p50/p90 regret 1.000×/1.00766×；同-family split 与 fair batching baseline 待补 | IR-5C2 v2 有 16 candidate measurements/32 outcomes；只支持 reduced MLP，不升级 paper-level claim |
...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4 validated-reduced closure；IR-5C2 MLP performance PARTIAL | composite typed registry + PR-13 manager→typed compiler adapter + real fused/unfused/fallback + exact state task skip；legacy α/β 默认关闭 | typed MLP CUDA 16/16 correct；独立 workload-family 与 fair batching 待补 | dispatch/query/state artifacts + IR-5C2 v2 可 replay；PR-13 α/β compiler migration 明确不成立 |
