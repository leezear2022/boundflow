...
| C2：Method/Autograd/Memory-Aware Materialization Planner | IR-4 closure + IR-5A adaptive mechanism validated-reduced；IR-5 held-out pending | typed PlanTemplate/PlanInstance、Task/Schedule lowering、state validity、query-time memory/deadline/cache/distribution 与 amortized selector | budget/cache/repeated context 切换合法 plan；fixed/local/global/oracle 公平回归待补 | 历史 1,416 executions/23 feasible 保留；PR-11/12 raw decision records 当前为 0，新 IR-5 必须独立产出 |
...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4 validated-reduced closure；IR-5 performance evidence pending | composite typed registry + PR-13 manager→typed compiler adapter + real fused/unfused/fallback + exact state task skip；legacy α/β 默认关闭 | MLP/CNN PyTorch/TVM、structured/chunked、cache/fallback、query order、state hit/stale、historical opt-in 与 external No-Go | dispatch/cache v2 与 query/state artifact v2 可 fresh-process replay；PR-13 α/β compiler migration 明确不成立 |
