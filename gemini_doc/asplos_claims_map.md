...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4B PyTorch dense/structured/chunked typed migration validated；TVM/query runtime pending | BackendDispatchKey + backend-specific Task identity + real fused registry；历史 TVM executor 尚待接入 | cache/stale/capability + MLP/CNN fused、structured、CUDA chunked 对齐 | typed dispatch key 已入 artifact；TVM compile/cold/warm、launch、bytes 待迁移 |
