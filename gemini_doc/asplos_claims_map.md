...
| BoundFlow Schedule IR | IR-3B control/executor/trace foundation validated；Task linkage 已接入、per-task semantics pending | typed ScheduleModule + batch/event/state/retry/replan + Task launch linkage；旧 topo loop 仅作对照 | 10 个 lowering/control/trace/artifact tests；OOM bounded、stream happens-before、query accounting、Bound oracle 对齐 | deterministic dump/hash/trace/fresh-process replay 已有；逐 Task/backend E2E 尚缺 |
| BoundFlow Task IR | IR-3C typed schema/lowering/linkage foundation validated；per-task semantics pending | TaskIRModule/Unit + typed op/parameter/external/state/memory/backend refs + Schedule linkage | 4 个 deterministic lowering、dependency、missing-param、launch-linkage tests | task module/dispatch trace 已入 fresh-process artifact；数学输出仍由 whole-Bound oracle 提供 |
