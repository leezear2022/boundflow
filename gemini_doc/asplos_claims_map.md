...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4C PyTorch+TVM typed migration validated；query/state/legacy entry pending | composite typed registry + backend-specific Task identity + real fused/unfused execution + semantic fallback | MLP/CNN PyTorch/TVM、structured/chunked、cache/fallback/capability 对齐 | dispatch key 已入 artifact/cache v2；fresh-process miss→disk-hit 已有，完整 query E2E 待迁移 |
