...
| C2：Method/Autograd/Memory-Aware Materialization Planner | IR-4 closure；IR-5C3 VALIDATED-NO-GO；IR-5D calibration-only；IR-5E/F v2 protocol-invalid | typed Plan/Task/Schedule、adaptive/fair evaluator、prepared execution、chain→residual CUDA suite | 正式旧 p90 70.263×；旧 CNN remediation median 0.880×/0.896×；v2 fixed-single 输入身份失败，无性能结果 | IR-5C3 仍是最后完整 artifact；7401/7402 退役，只允许显式 input slice 的新 protocol |
