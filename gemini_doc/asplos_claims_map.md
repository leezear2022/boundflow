...
| BoundFlow Schedule IR | IR-3A schema/lowering/verifier foundation validated；executor/control actions pending | typed ScheduleModule + budget/allocate/materialize/launch/emit/free；旧 topo loop 仅作对照 | 3 个 deterministic lowering、memory/use-def/query accounting tests；仍缺 stream/batch/retry/state/executor | deterministic dump/hash 已有；trace/replay/E2E 尚缺 |
