...
| C2：Method/Autograd/Memory-Aware Materialization Planner | IR-4 closure；IR-5C3 VALIDATED-NO-GO；IR-5D calibration-only；v2 invalid；v3 protocol frozen | typed Plan/Task/Schedule、adaptive/fair evaluator、prepared execution、exact-input chain→residual CUDA suite | 正式旧 p90 70.263×；旧 CNN remediation median 0.880×/0.896×；v3 7501/7502 未执行 | IR-5C3 仍是最后完整 artifact；v3 固化 from-trace/input identity/p90/Pareto，等待一次性 final |
...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4 validated-reduced；IR-5C3 fair performance No-Go；IR-5D calibration-only remediation | composite typed registry + query adapter + real fused/unfused/fallback；prepared capsule 将静态 validate/hash/dispatch 移出 query hot path | 正式 typed Global 对 batched-original p90 regret 70.263×；旧 CNN from-trace calibration 低于 1× | IR-5C3 artifact 可 replay；IR-5D 新 residual final 尚缺，production performance claim 仍不成立 |
