...
| backend 执行 typed Planner/Task 结果而非定义核心抽象 | IR-4A PyTorch reference dispatch foundation；chunked/structured/TVM pending | typed BackendDispatchKey + PyTorch reference adapter；历史 TVM executor 尚待接入 | cache hit/stale/capability rejection + Python/TVM/unfused/fused 对齐 | typed dispatch key 已入 artifact；TVM compile/cold/warm、launch、bytes 待迁移 |
